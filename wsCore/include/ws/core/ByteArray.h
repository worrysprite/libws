...
namespace ws
{
	namespace core
	{
		...
		public:
			...
			//获取管理的内存块
			inline void* getBytes(){ return data; }

			//获取当前读位置的指针
			inline const void* readerPointer() const { return (void*)((intptr_t)data + _readPos); }
			//获取当前写位置的指针
			inline void* writerPointer() { return (void*)((intptr_t)data + _writePos); }
