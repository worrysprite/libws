...
namespace ws::core::TimeTool
{
	...
	template<class ClockType = std::chrono::system_clock, class Duration = std::chrono::milliseconds>
		requires std::chrono::is_clock_v<ClockType>&& is_duration_v<Duration>
	inline time_t getUnixtime() { return std::chrono::duration_cast<Duration>(ClockType::now().time_since_epoch()).count(); }

	//获取系统时间戳（毫秒）
	inline time_t getSystemTime() { return getUnixtime(); }
